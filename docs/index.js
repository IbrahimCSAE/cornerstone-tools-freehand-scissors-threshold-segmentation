...
const imageIds = [
  ...
  const display = async (element, imageIds) => {
    ...
    cornerstoneTools.addToolForElement(
      ...
      {configuration : {thresholdLow: 200, thresholdHigh: 1000, inside: true, numberOfSlices: 10}}
    );
    cornerstoneTools.setToolActive("StackScrollMouseWheel", {});

    cornerstoneTools.setToolActiveForElement(element, "FreehandScissorsThresholdSegmentation",{ mouseButtonMask: 1})
    return Promise.all(
  ...
  ;(async function () {
    ...
    min.addEventListener("change", (e) => {
      cornerstoneTools.store.state.tools.filter(tool => tool.name == 'FreehandScissorsThresholdSegmentation')[0].configuration.thresholdLow = e.target.value
      cornerstone.updateImage(axial)
    })

    max.addEventListener("change", (e) => {
      cornerstoneTools.store.state.tools.filter(tool => tool.name == 'FreehandScissorsThresholdSegmentation')[0].configuration.thresholdHigh = e.target.value
      cornerstone.updateImage(axial)
    })

    slices.addEventListener("change", (e) => {
      cornerstoneTools.store.state.tools.filter(tool => tool.name == 'FreehandScissorsThresholdSegmentation')[0].configuration.numberOfSlices = e.target.value
      cornerstone.updateImage(axial)
    })

    fill.addEventListener("change", (e) => {
      cornerstoneTools.store.state.tools.filter(tool => tool.name == 'FreehandScissorsThresholdSegmentation')[0].configuration.inside = e.target.checked
      cornerstone.updateImage(axial)
    })
  })()